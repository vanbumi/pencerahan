void function ( $, _ ) {
	...
	function tap ( e ) {
		if (e.target.classList.contains('es-draggable')) {
			target = $(e.target);
		}
		else {
	...
	function drag ( e ) {
		...
		if ( !dragging ) {
			...
			$('body').add(target).addClass('es-draggable-dragging');
		}
		if ( dragging ) {
	...
	function release ( e ) {
		...
		dragging = false;
		$('body').add(target).removeClass('es-draggable-dragging');
		target.trigger(createEvent('es_dragstop', e), getData());
		target.removeData('es-draggable-data');
	}
	...
	function findParentNode( child ) {
		...
		while (parent && !found && !parent.isEqualNode(document.body))
			parent.classList && parent.classList.contains('es-draggable') ?
				found = parent:
