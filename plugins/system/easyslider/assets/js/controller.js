void function ( exports, $, _, Backbone ) {
		...
		slider.on('slide:transition', function ( slide, direction, side, progress ) {
			...
			switch ( direction ) {
				...
				case 'out': // The active slide facing front
					...
					slide.$el.css('transform', 'translate3d(0,0,' + (width / 2) + 'px)');
					$cube.css('transform', 'translate3d(0,0,' + (-width / 2) + 'px) rotate3d(0,1,0,' + rotation + 'deg)');
					break;

				case 'in': // The next slide facing left or right
					var faceRotation = side == 'left' ? -90 : 90;
					var faceOffsetX = (side == 'left' ? -width : width) / 2;
	...
	exports.ES_Carousel_Controller = function ( slider ) {
		...
		slider.on('resize', function () {
			...
			radius = getOuterRadius(slider.stage_width, slides.length);
			slider.setPerspective(slider.stage_width)
		});
