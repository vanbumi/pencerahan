...
rangy.createModule("Position", ["WrappedSelection"], function(api, module) {
    ...
    function pointIsInOrAboveRect(x, y, rect) {
        log("pointIsInOrAboveRect", x, y, Math.floor(rect.top), Math.floor(rect.right), Math.floor(rect.bottom), Math.floor(rect.left))
    ...
    function createRangeFromPoints(startX, startY, endX, endY, doc) {
        ...
        log(startPos.node, startPos.offset, endPos.node, endPos.offset);
        var range = api.createRange(doc);
                ...
                createClientBoundaryPosGetter = function(isStart) {
                    return function() {
                        ...
                        if (rects.length == 0 && elementSupportsGetBoundingClientRect) {
                            ...
                            log(nativeRange, nativeRange.getClientRects(), nativeRange.getBoundingClientRect());
                            if (this.collapsed
                                ...
                                if (n.getClientRects) {
                                    log(n, n.getClientRects(), this.startContainer.getClientRects())
                                }
